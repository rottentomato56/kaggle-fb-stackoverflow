...
#    for line in f:
#      counter1 += 1
#      sys.stdout.write('\r' + str(counter1))
#      sys.stdout.flush()
...
#      
#      final_index[tag] = [indexed_terms, class_length,  sorted_term_list]


  final_index = {}
  c = collection2.find({}, {'terms' : 1, 'tag' : 1, 'norm' : 1})
  for p in c:
    counter1 += 1
    sys.stdout.write('\r' + str(counter1))
    sys.stdout.flush()
    final_index[p['tag']] = [p['norm'], p['terms']]
  ...
  with open('/home/jason/Kaggle/Facebook/Data/Testing/Parsed/' + test_file) as f:
    for line in f:
      ...
      if not sorted_scores:
        predicted_tags = predict(post, final_index)
